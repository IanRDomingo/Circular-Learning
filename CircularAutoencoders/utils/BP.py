...
class BP:
    def __init__(self, model, autoencoding=False, init='wt', nofprime=False, skipped=False, device='cpu'):
        self.model = model
    ...
    def compute_direction(self, Xlist, target):
        ...
        for i in range(ifinal, -1, -1):
            if i == ifinal:
                ...
                if(type(upper_layer).__name__ == "TensorflowConvLayer"):
                    Wrbp = list(self.model.children())[i+1].conv.weight
                elif(type(upper_layer).__name__ == "TensorflowLinearLayer"):
                    Wrbp = list(self.model.children())[i+1].linear.weight
                if self.skipped:
                    if self.nofprime:
