...
class BasicConvModel(nn.Module):
    def __init__(self, inc, outc, ksiz, pad):
        ...
        self.C2 = TensorflowConvLayer(outc, inc, ksiz, stride=1, padding=pad, activation_fn=nn.Identity())
...
class DeeperConvModel(nn.Module):
    def __init__(self):
        ...
        self.C4 = TensorflowConvLayer(3, 3, 5, stride=1, padding="same", activation_fn=nn.Identity())
...
class DeeperLinearModel(nn.Module):
    def __init__(self, inshape, midshape, outshape):
        ...
        self.L4 = TensorflowLinearLayer(midshape, inshape, activation_fn=nn.Identity())
    def forward(self, x):
        x = self.I(x)
        x = self.L1(x)
        x = self.L2(x)
